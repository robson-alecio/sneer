...
import sneer.bricks.things.impl.Finder;
import sneer.bricks.things.impl.ThingImpl;


public class ThingsTest {

	private final List<ThingImpl> _apAds = new ArrayList<ThingImpl>();
	...
	private void find(String tags, int thingsToFind) {
		...
		Collection<ThingImpl> found = finder.find(_apAds, tags);
	...
	private void createApartmentAds() {
		...
		_apAds.add(new ThingImpl(name, description));
