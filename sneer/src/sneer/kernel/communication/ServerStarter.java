...
class ServerStarter implements Receiver<Integer> {
	...
	public static void start(User user, OldNetwork network, Signal<Integer> sneerPort) {
		new ServerStarter(user, network, sneerPort);
	}

	private ServerStarter(User user, OldNetwork network, Signal<Integer> sneerPort) {
		_user = user;
		_network = network;
